...
def label_process(labels, n_classes,train_data, noise_type='uniform', noise_rate=0, random_seed=5, debug=False):
    '''
    ...
    if noise_rate > 0.0:
        noisy_labels = add_label_noise(labels[train_data].cpu().numpy(), cp, random_seed)
        noisy_train_labels = torch.tensor(noisy_labels).to(labels.device)
    else:
        if debug:
            print('Clean data')
        noisy_train_labels = labels[train_data]


    if debug:
        #print('#Actual noise rate %.2f ' % actual_noise_rate)
        pass
    return noisy_train_labels, None, None
